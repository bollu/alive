...
(* precondition: true *)
(*
 let %c = icmp uge %A, %B in 
 let %umax = select i1 %c, %A, %B in 
 let %c2 = icmp uge %umax, %B in 
 let %umax2 = select i1 %c2, %umax, %B in 

=>
 let %c = icmp uge %A, %B in 
 let %umax = select i1 %c, %A, %B in 
 let %c2 = icmp uge %umax, %B in 
 let %umax2 = select i1 %c, %A, %B in 

*)
let alive_Select_699 (w : pos) (input_b input_a : BV.bv_t 1)
 : Lemma ((
  let var_0 = () in
  let var_1 = op_const (input_a) var_0 in
  let var_2 = op_const (input_b) var_0 in
  let var_3 = (var_1, var_2) in
  let var_4 = op_icmp uge  w var_3 in
  ...
  let var_2 = op_const (input_b) var_0 in
  let var_3 = (var_1, var_2) in
  let var_4 = op_icmp uge  1 var_3 in
  let var_5 = triple:var_4 var_1 var_2 in
  let var_6 = op_select 1 var_5 in
  let var_7 = (var_6, var_2) in
  let var_8 = op_icmp uge  1 var_7 in
