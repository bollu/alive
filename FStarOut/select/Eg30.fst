...
(* precondition: true *)
(*
 let %c = icmp eq %X, C in 
 let %r = select i1 %c, %X, %Y in 

=>
 let %c = icmp eq %X, C in 
 let %r = select i1 %c, C, %Y in 

*)
let alive_Select_637 (w : pos) (input_c input_y : BV.bv_t 1)
(input_x : BV.bv_t w)
 : Lemma ((
  let var_0 = () in
  ...
  let var_6 = triple:var_4 var_1 var_5 in
  ...
  let var_4 = op_icmp eq  w var_3 in
  let var_5 = op_const (input_y) var_0 in
  let var_6 = triple:var_4 var_2 var_5 in
  let var_7 = op_select w var_6 in
  (* return_value *) var_7))
 = ()